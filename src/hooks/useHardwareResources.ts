...
function useHardwareResources() {
    ...
    const accessDefaultCamereaAndMicrophone = async (permission: { video: boolean, audio: boolean }) => {
        const stream = await navigator.mediaDevices.getUserMedia(permission);
        videoRef.current!.srcObject = stream;
        setStream(stream);
    }

    const handleUserPermissions = (video: boolean, audio: boolean) => setUserPermission({ video, audio });
